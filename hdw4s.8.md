...
## FILES
  ...
  * `/etc/hdw4s/handover.off`:
    Present only if session hand-over has been rolled back. The updater applies
    the client half every time it runs, so without this a rollback would last
    until the next run and no longer. Created by rolling back, removed by
    applying again, and safe to delete by hand to let the updater apply it.
